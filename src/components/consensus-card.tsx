    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="font-display mt-1 text-xl font-medium">{item.label}</h3>
          <p className="mt-1 truncate text-sm text-muted-foreground">
            {f.away.name} at {f.home.name} · {f.league} · {formatKickoff(f.start)}
          </p>
        </div>
        <Badge variant={item.pct >= 0.75 ? "win" : item.pct >= 0.6 ? "info" : "default"}>
      ...
      <p className="mt-3 text-xs break-words text-subtle">
        ...
        {item.fade.length ? (
          ...
          <span className="mt-1 block text-win">Unanimous on the desk</span>
        )}
      </p>
    </Link>
  );
